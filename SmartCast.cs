...
namespace SmartCast
{
    public unsafe class SmartCast : IDalamudPlugin
    {
        ...
        private delegate long GetAdjustedActionIdDelegate(IntPtr a1, uint a2);
        ...
        private unsafe delegate byte DoActionLocationDelegate(long a1, uint actionType, uint actionId,
            long a4 /* 0xE0000000 */, Vector3* a5, uint a6 /* 0 */);
        ...
        private Hook<MouseToWorldDelegate> _mouseToWorldHook;
        ...
        public SmartCast(DalamudPluginInterface pluginInterface)
        {
            ...
            GroundTargetActions = DalamudApi.DataManager.GetExcelSheet<Action>()
                .Where(i => i.TargetArea && i.RowId != 7419 && i.RowId != 3573).ToDictionary(i => i.RowId, j => j);
            DismountActions = DalamudApi.DataManager.GetExcelSheet<Action>().Where(i =>
                    i.IsPlayerAction && i.RowId > 8 && i.ActionCategory?.Value?.RowId is 2 or 3 or 4 or 9 or 15)
                .ToDictionary(i => i.RowId, j => j);
            BattleJobs = DalamudApi.DataManager.GetExcelSheet<ClassJob>()
                .Where(i => i.ClassJobCategory?.Value?.RowId is 30 or 31).Select(i => i.RowId).ToHashSet();

            actionManager =
                (ActionManager*)DalamudApi.SigScanner.GetStaticAddressFromSig(
                    "48 8D 0D ?? ?? ?? ?? E8 ?? ?? ?? ?? 8B F8 8B CF");
            _canCastFunc = DalamudApi.SigScanner.ScanText("E8 ?? ?? ?? ?? 83 BC 24 ?? ?? ?? ?? ?? 8B F0");
            _doActionFunc = DalamudApi.SigScanner.ScanText("E8 ?? ?? ?? ?? EB 64 B1 01");
            _mouseToWorldFunc =
                DalamudApi.SigScanner.ScanText("E8 ?? ?? ?? ?? 48 8D 0D ?? ?? ?? ?? E8 ?? ?? ?? ?? 84 C0 74 4E");
            ...
            _getAdjustedActionId =
                Marshal.GetDelegateForFunctionPointer<GetAdjustedActionIdDelegate>(_getAdjustedActionIdFunc);
            ...
            mouseOverUiHook = new Hook<MouseOverUi>(
                DalamudApi.SigScanner.ScanText("40 56 57 41 55 48 83 EC 30 4C 89 64 24 ??"),
                MouseOverUiDeto);
            ...
            DalamudApi.Framework.Update += OnFramework;
            if (pluginInterface.Reason is not PluginLoadReason.Boot or PluginLoadReason.Update) pluginUI.Visible = true;
        }
        ...
        private void toggle(string displayName, string input, ref bool value)
        {
        ...
        private (long a1, uint actionType, uint actionId, long targetId, int a5, uint castFrom, int a7, long a8)?
            waitedAction = null;
        ...
        private void OnFramework(Dalamud.Game.Framework framework)
        {
            ...
            var (a1, actionType, actionId, targetId, a5, castFrom, a7, a8) =
                waitedAction.Value;

            PluginLog.Verbose(
                $"Cast queue: actionType: {actionType}, actionId: {actionId}, targetId: {targetId:X}, castFrom: {castFrom}, a5: {a5}, a7: {a7}");
            ...
            if (actionReady == 0)
            {
                ...
                if (cancast == 0)
                {
                    ...
                    PluginLog.Debug(
                        $"Cancast passed: DoAction ret: {doaction}, actionType: {actionType}, actionId: {actionId}, targetId: {targetId:X}, castFrom: {castFrom}, a5: {a5}, a7: {a7}");
                    if (doaction == 1)
                    {
                        waitedAction = null;
                        tryTime = null;
        ...
        private void mousetoworld(long a1, uint spellid, uint a3, long result)
        {
            PluginLog.Debug($"MouseToWorld:{spellid}:{a3}");
            var str = "";
            for (var i = 0; i < 30; i++) str += " " + Marshal.ReadByte((IntPtr)result, i);
        ...
        private uint GetObjectId(uint index)
        {
            ...
            if (ptr != IntPtr.Zero || (int)ptr != 0) return (uint)Marshal.ReadInt32(ptr + +0x74);
        ...
        private void QueueAction(long a1, uint type, uint adjustedId, uint targetId, uint castFrom)
        {
        ...
        {
            PluginLog.Debug(
                $"DoAction original: A1:{a1:X}, actionType: {actionType}, actionId: {actionId}, targetId: {targetId:X}, castFrom: {castFrom}, a5: {a5}, a7: {a7}, a8:{a8}");
            ...
            if (config.AutoDismount)
            {
                ...
                if (mounted && (ActionType)actionType == ActionType.Spell)
                    if (DismountActions.ContainsKey(adjustedId) && DalamudApi.ClientState.LocalPlayer is not null &&
                        BattleJobs.Contains(DalamudApi.ClientState.LocalPlayer.ClassJob.Id))
                    {
                        ...
                        PluginLog.Debug(
                            $"Used actionType: {actionType}, actionId: {actionId}->{adjustedId}, Dismounting!");
            ...
            else if (_actionReady(a1, actionType, actionId) == 0L) //可以使用技能
            {
                if (castFrom == 1) //队列中技能
                {
                    ...
                    {
                        var pos = queuedPos;
                        var b = _doActionLocation(a1, actionType, actionId, targetId, &pos, 0U);
                ...
                if (castFrom != 2 && queuedPos == Vector3.Zero && queuedTarget == 0xE000_0000) //非宏，无储存
                    if (config.EnableSmartCast && GroundTargetActions.TryGetValue(actionId, out var action))
                        if (config.GroundTargetSmartCastForNonPlayerSpell || action.IsPlayerAction)
                        {
                            ...
                            {
                                var b = _doActionLocation(a1, actionType, actionId, targetId, &worldPos,
                                    0U);
                                PluginLog.Debug($"_doActionLocation ret: {b}");
                                return b;
                            }
            ...
            //Smart悬浮施法
            if (config.MouseOverFriendly && DismountActions.TryGetValue(actionId, out var action2) &&
                (action2.CanTargetFriendly || action2.CanTargetParty))
            {
            ...
            {
                var original = _doActionHook.Original(a1, actionType, actionId, targetId, a5, castFrom, a7, a8);
                PluginLog.Debug(
                    $"DoAction ret: original: A1:{a1:X}, actionType: {actionType}, actionId: {actionId}, targetId: {targetId:X}, castFrom: {castFrom}, a5: {a5}, a7: {a7}, a8:{a8}");
                return original;
            }
        }

        private void MouseToWorld(long a1, uint spellId, uint actionType, out bool mouseOnWorld, out bool success,
